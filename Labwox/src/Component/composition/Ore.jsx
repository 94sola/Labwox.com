import React, { useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import { Gem, Mountain, FlaskConical, Microscope, ArrowLeft } from "lucide-react";
import Wrapper from "../wrapper";

// Ore sample categories
const samples = [
  {
    name: "Iron Ores",
    icon: <Mountain className="w-10 h-10 text-[#FFC000]" />,
    description:
      "Quantitative analysis of hematite, magnetite, and other iron-bearing ores for Fe, SiO₂, Al₂O₃, and trace impurities.",
    link: "/ores/iron",
  },
  {
    name: "Copper Ores",
    icon: <FlaskConical className="w-10 h-10 text-[#FFC000]" />,
    description:
      "Elemental profiling of chalcopyrite, bornite, and malachite ores to determine Cu, Fe, S, and associated trace metals.",
    link: "/ores/copper",
  },
  {
    name: "Bauxite Ores",
    icon: <Gem className="w-10 h-10 text-[#FFC000]" />,
    description:
      "Analysis of bauxite ores for Al₂O₃, SiO₂, Fe₂O₃, TiO₂, and loss on ignition (LOI) to assess grade and quality.",
    link: "/ores/bauxite",
  },
  {
    name: "Rare Earth & Specialty Ores",
    icon: <Microscope className="w-10 h-10 text-[#FFC000]" />,
    description:
      "Determination of lanthanides, lithium, cobalt, and other critical elements in rare earth and specialty ores.",
    link: "/ores/rare-earth",
  },
];

// Elements commonly tested in ores
const compounds = [
  "Iron (Fe)",
  "Aluminium (Al)",
  "Silicon (Si)",
  "Calcium (Ca)",
  "Magnesium (Mg)",
  "Titanium (Ti)",
  "Manganese (Mn)",
  "Sulfur (S)",
  "Phosphorus (P)",
  "Copper (Cu)",
  "Cobalt (Co)",
  "Nickel (Ni)",
  "Zinc (Zn)",
  "Lead (Pb)",
  "Rare Earth Elements (REEs)",
];

const Ore = () => {
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();

  const handleClick = (path) => {
    setLoading(true);
    setTimeout(() => {
      navigate(path);
      setLoading(false);
    }, 500);
  };

  return (
    <Wrapper>
      <section
        className="bg-gradient-to-b from-white via-neutral-50 to-white py-16 lg:py-24 mb-10"
        style={{ cursor: loading ? "wait" : "default" }}
      >
        {/* Back Link */}
        <div className="max-w-5xl mx-auto px-4 mb-8">
          <Link
            to="/composition"
            className="inline-flex items-center gap-2 text-[#153D63] italic hover:text-[#FFC000] transition-colors duration-200 font-medium"
          >
            <ArrowLeft className="w-5 h-5" /> Back to compositions application
          </Link>
        </div>

        {/* Header */}
        <div className="max-w-5xl mx-auto text-center mb-12 px-4">
          <h1 className="text-5xl md:text-7xl font-light text-[#153D63] mb-6">
            Elemental Analysis of Ores
          </h1>
          <p className="text-xl text-gray-700 leading-relaxed">
            At <strong>ChemXpert</strong>, we provide accredited{" "}
            <em>elemental analysis</em> of ores to support mining, metallurgy,
            and environmental applications. Using advanced instrumentation
            (ICP-OES, XRF, AAS), we quantify major, minor, and trace elements
            with precision to ensure accurate ore characterization.
          </p>

          {/* Compounds List */}
          <div className="mt-12">
            <h3 className="text-2xl font-semibold text-[#FFC000] mb-6 text-center">
              Elements Analyzed
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {compounds.map((compound, index) => (
                <div
                  key={index}
                  className="border border-gray-200 bg-white rounded-xl shadow-sm p-4 text-center text-gray-700 hover:border-[#FFC000] hover:shadow-lg transition-all duration-300"
                >
                  {compound}
                </div>
              ))}
            </div>
          </div>
        </div>

        {/* Sample Types */}
        <h2 className="text-4xl font-normal text-[#153D63] text-center mb-8">
          Select Sample Type
        </h2>
        <div className="max-w-7xl mx-auto px-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {samples.map((sample, index) => (
            <div
              key={index}
              onClick={() => handleClick(sample.link)}
              style={{ cursor: loading ? "wait" : "pointer" }}
              className="bg-white p-6 rounded-2xl shadow-lg hover:shadow-xl border border-gray-200 hover:border-[#FFC000] transition-all duration-300 text-center flex flex-col items-center h-full group"
            >
              {sample.icon}
              <h3 className="text-2xl font-normal text-gray-800 mt-4 mb-2 group-hover:text-[#FFC000] transition-colors duration-200">
                {sample.name}
              </h3>
              <p className="text-gray-600 text-base leading-relaxed">
                {sample.description}
              </p>
            </div>
          ))}
        </div>
      </section>
    </Wrapper>
  );
};

export default Ore;
