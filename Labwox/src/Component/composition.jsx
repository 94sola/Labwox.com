    ...
    <Wrapper>
      <section className="relative w-full py-16 lg:py-24 bg-gradient-to-b from-black via-neutral-950 to-black overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 px-4">
          {[
            { name: "Proximate Analysis", link: "/proximate" },
            { name: "Nutritional Composition", link: "/nutritional" },
            { name: "Moisture Content", link: "/moisture" },
            { name: "Protein Content", link: "/protein" },
            { name: "Fat Content", link: "/fat" },
            { name: "Ash Content", link: "/ash" },
          ].map((item, index) => (
            <Link
              key={index}
              to={item.link}
